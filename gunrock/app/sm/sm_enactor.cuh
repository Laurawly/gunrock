...
struct SMIterationLoop : public IterationLoopBase<EnactorT, Use_FullQ | Push> {
  ...
  cudaError_t Core(int peer_ = 0) {
    ...
    // advance to neighbors
    auto advance_next = [subgraphs, constrain, isValid] __host__ __device__(
                            const VertexT &src, VertexT &dest,
                            const SizeT &edge_id, const VertexT &input_item,
                            const SizeT &input_pos, SizeT &output_pos) -> bool {
      if (isValid[src]) {
        if (subgraphs[dest] >= constrain[0]) {
          return true;
        }
      }
      return false;
    ...
    /* auto look_ahead_op =
         [isValid, flags, results, NS, counter, subgraphs, nodes_data,
          nodes_query] __host__
         __device__(const VertexT &src, VertexT &dest, const SizeT &edge_id,
                    const VertexT &input_item, const SizeT &input_pos,
                    SizeT &output_pos) -> bool {
       VertexT query_id = NS[counter[0]];
       SizeT min_degree = NS[counter[0] + nodes_query];
       if (src >= nodes_data) return false;
       if ((!isValid[src]) || (!isValid[dest])) {
         return false;
       }
       // src is valid
       if (flags[src] == 0) return false;
       if (src > dest) return false;
       // 1 way look-ahead
       if (subgraphs[dest] < min_degree) return false;
       flags[src] = 0;
       if (flags[dest] == 0 || flags[dest] > src + 1) flags[dest] = src + 1;
       return true;
     }; // look_ahead_op
    ...
    for (int iter = 0; iter < nodes_query; ++iter) {
      ...
      GUARD_CU(counter.ForAll(
          [iter] __host__ __device__(VertexT * counter_, const SizeT &pos) {
            counter_[pos] = iter;
          },
          1, target, stream));
      counter.Print();
   ...
   GUARD_CU(util::CUBSelect_flagged(
      ...
      counter.GetPointer(util::DEVICE),
      nodes_data));
    indices.Print();
    results.Print();
    counter.Print();*/
   ...
   * @param  received_length The number of transmition items received
